...
public class ConferenceTalkAccessor extends ConferenceSubEventAccessor {

    public ConferenceTalkAccessor(OntModel model) {
        super(model);
    }
    ...
    public void associateTimeInterval(Attribute attribute, TimeInterval timeInterval) throws AccesorInitializationException {
        throw new AccesorInitializationException("TimeInterval value being initialized for wrong attribute "
                + ConferenceTalkAccessor.class.getName());
    }
    ...
    public IResultSet executeQuery() throws SourceQueryException {
        BasicDBList results = query("talks");
        try {
            return convert(results, model.getOntClass(Constants.CuenetNamespace + "talk"));
        } catch (IOException e) {
            e.printStackTrace();
        }
