...
public class KeynoteAccessor extends ConferenceSubEventAccessor {

    public KeynoteAccessor(OntModel model) {
        super(model);
    }
    ...
    public IResultSet executeQuery() throws SourceQueryException {
        BasicDBList results = query("keynotes");
        try {
            return convert(results, model.getOntClass(Constants.CuenetNamespace + "keynote"));
        } catch (IOException e) {
            e.printStackTrace();
        }
